...
Tu bâtis des fronts Angular et des back .NET soumis au RGPD. Rampart te donne une **barrière PII côté client**, avant tout appel à une API LLM tierce : minimisation des données par conception, sans serveur d'inférence à héberger ni coût GPU. Utile aussi pour anonymiser des logs ou des tickets avant analyse.

### Limites

Un MiniLM quantifié privilégie la vitesse : le **rappel** sur des formats rares (IBAN exotiques, noms composés) reste imparfait. Garde-le comme **défense en profondeur**, pas comme unique garde-fou de conformité. Vérifie aussi le premier chargement (quelques Mo) sur mobile.
